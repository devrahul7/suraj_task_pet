...
import { BookAppointmentDto, UpdateAppointmentStatusDto } from "../dtos/vet-appointment.dto";
...
export class VetAppointmentService {
    ...
    async bookAppointment(userId: string, dto: BookAppointmentDto) {
        ...
        if (!vet.isActive) {
            throw new HttpException(400, "This veterinarian is not currently accepting appointments");
        }
        ...
        if (appointmentDate < now) {
            throw new HttpException(400, "Appointment date must be in the future");
        }
        ...
        const slotTaken = await this.appointmentRepository.isSlotTaken(
            dto.veterinarianId,
            appointmentDate,
            timeSlot
        );
        if (slotTaken) {
            throw new HttpException(409, "This time slot is already booked. Please choose another.");
        }

        // 7. Create appointment
        const appointment = await this.appointmentRepository.create({
            userId: userId as any,
            veterinarianId: dto.veterinarianId as any,
            petName: dto.petName,
        ...
        void this.notificationService
            .createNotification(
                ...
                "system",
                "Appointment Booked",
                `Your appointment with Dr. ${vet.name} on ${appointmentDate.toDateString()} at ${timeSlot} has been submitted. Awaiting confirmation.`,
    ...
    async getMyAppointments(userId: string, page = 1, limit = 10) {
        const { appointments, total } = await this.appointmentRepository.findByUserId(
            userId,
            page,
            limit
        );
     ...
     */
    async getAppointmentById(id: string, userId: string, isAdmin = false) {
        ...
        if (!isAdmin && appointment.userId.toString() !== userId) {
            throw new HttpException(403, "You do not have permission to view this appointment");
        }
     ...
     */
    async cancelAppointment(id: string, userId: string, cancellationReason?: string) {
        const appointment = await this.appointmentRepository.findById(id);
    ...
    async updateAppointmentStatus(id: string, dto: UpdateAppointmentStatusDto) {
        ...
        if (appointment.status === "COMPLETED") {
            throw new HttpException(400, "Cannot change status of a completed appointment");
        }
        if (appointment.status === "CANCELLED") {
            throw new HttpException(400, "Cannot change status of a cancelled appointment");
        }
        ...
        const updated = await this.appointmentRepository.update(id, updateData as any);
                ...
                if (dto.status === "CONFIRMED") {
                    ...
                    void this.notificationService
                        .createNotification(
                            ...
                            `Your appointment with ${vet.name} on ${updated.appointmentDate.toDateString()} at ${updated.timeSlot} has been confirmed.`,
                            "/dashboard/user/appointments",
                            { appointmentId: id }
                        )
                        .catch((error) => {
                ...
                if (dto.status === "CANCELLED") {
                    ...
                    void this.notificationService
                        .createNotification(
                            ...
                            "system",
                            "Appointment Cancelled",
                            `Your appointment with ${vet.name} on ${updated.appointmentDate.toDateString()} at ${updated.timeSlot} has been cancelled.`,
                            "/dashboard/user/appointments",
                            { appointmentId: id }
                        )
                        .catch((error) => {
                ...
                if (dto.status === "COMPLETED") {
                    ...
                    void this.notificationService
                        .createNotification(
                            ...
                            `Your appointment with ${vet.name} on ${updated.appointmentDate.toDateString()} at ${updated.timeSlot} has been completed.`,
                            "/dashboard/user/appointments",
                            { appointmentId: id }
                        )
                        .catch((error) => {
     ...
     */
    private validateSlotWithinAvailability(
        vet: { availability: { day: string; startTime: string; endTime: string }[] },
        ...
        if (!isWithinWindow) {
            throw new HttpException(
                400,
                `Time slot ${timeSlot} is outside the veterinarian's availability on ${dayName}`
            );
        }
    }
}
